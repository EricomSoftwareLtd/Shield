...
function download_addnodes_py() {
    local MAIN_SCRIPT_URL="https://raw.githubusercontent.com/EricomSoftwareLtd/Shield/$BRANCH/Setup/addnodes.py"
    local RETURN_CODE="$(curl -so ./addnodes.py -w '%{response_code}' "$MAIN_SCRIPT_URL")"
...
if [ ! -r "./addnodes.py" ]; then
    download_addnodes_py
fi

exec python3 ./addnodes.py $@
